-- Migration: 20260906010000_harden_strategy_stability_publication.sql
-- Description: Atomic publication transaction RPC with concurrency guard and rollback protection.
-- Guarantees:
-- 1. Atomically supersedes expected current published strategy and inserts new version.
-- 2. If insertion fails, supersession rolls back automatically (zero-published prevention).
-- 3. Concurrent publications fail deterministically if expectedCurrentStrategyId does not match.
-- 4. Server-only execution; strictly non-private.
...
COMMENT ON FUNCTION public.publish_strategy_version_atomic(JSONB, TEXT) IS
    'Atomically supersedes current published strategy and inserts new strategy version within a single transaction. Strictly non-private.';
