...
/**
 * Transitional resolver explicitly for legacy single-owner authentication.
 * Resolves the unowned legacy profile (user_id IS NULL), falling back to
 * the earliest created profile for backward-compatibility with unmigrated DB.
 */
export async function getLegacyOwnerProfile(client = privateSupabase) {
  const db = client || privateSupabase;
  if (!db) throw new Error('Private database access is not configured');

  // First attempt: resolve specifically unowned profile (user_id IS NULL)
  const { data, error } = await db
    .from('investor_profile')
  ...
  // Fallback for pre-migration schema where user_id column does not exist yet (code 42703)
  // or if no unowned profile found (legacy singleton fallback)
  ...
  if (fallback.error) {
    throw new Error(`Database query error: ${fallback.error.message} (code: ${fallback.error.code || 'UNKNOWN'})`);
  }

  return fallback.data ? normalizeProfile(fallback.data) : null;
}

/**
 * Feature 13C: Checks if an unclaimed legacy profile exists without leaking
 * financial values, balances, or identifiers.
 */
export async function hasUnclaimedLegacyProfile(client = privateSupabase) {
  const db = client || privateSupabase;
  if (!db) throw new Error('Private database access is not configured');
  ...
  if (error) {
    ...
    throw new Error(`Failed to check legacy profile availability: ${error.message}`);
  }
...
/**
 * Feature 13C: Atomically claims the single unowned legacy profile for an authenticated user.
 */
export async function claimLegacyProfile(userId, client = privateSupabase) {
  const db = client || privateSupabase;
  if (!db) throw new Error('Private database access is not configured');
  ...
  if (typeof db.rpc === 'function') {
    ...
    if (error) {
      const err = new Error(`Legacy claim error: ${error.message} (code: ${error.code || 'UNKNOWN'})`);
      err.code = error.code || 'UNKNOWN';
      if (error.code === 'IP004') err.statusCode = 400;
      if (error.code === 'IP005') err.statusCode = 409;
      if (error.code === 'IP006') err.statusCode = 410;
      throw err;
    }
  ...
  // 1. Verify user does not already have a profile
  const existing = await getProfileByUserId(userId, db);
  if (existing) {
    const err = new Error('User already has an assigned profile');
    err.code = 'IP005';
    err.statusCode = 409;
    throw err;
  }

  // 2. Atomically update legacy profile where user_id IS NULL
  const { data, error } = await db
    .from('investor_profile')
  ...
  if (error) {
    throw new Error(`Failed to claim legacy profile: ${error.message}`);
  }

  if (!data || data.length === 0) {
    const err = new Error('No unclaimed legacy profile available');
    err.code = 'IP006';
    err.statusCode = 410;
    throw err;
  }
