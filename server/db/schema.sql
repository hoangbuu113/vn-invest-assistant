...
CREATE TABLE IF NOT EXISTS public.assets (
    ...
    fundamentals_company_type VARCHAR(16) CHECK (
...
-- ==========================================================
-- Schema Migration: 20260913000000_create_verified_equity_fundamentals.sql
-- Purpose: Immutable manually verified official-filing fundamentals for VN industrial equities
-- ==========================================================
...
BEGIN
    IF jsonb_typeof(p_filing) IS DISTINCT FROM 'object'
       ...
       OR jsonb_array_length(p_facts) = 0 THEN
        RAISE EXCEPTION 'Filing and a non-empty facts array are required' USING ERRCODE = '22023';
    END IF;
    ...
    IF v_supersedes_id IS NOT NULL THEN
        ...
        IF v_previous.asset_id IS DISTINCT FROM (p_filing ->> 'asset_id')::UUID
           ...
           OR EXISTS (
               SELECT 1 FROM public.vn_equity_fundamental_filings
    ...
    INSERT INTO public.vn_equity_fundamental_filings (
        ...
        v_id, p_filing ->> 'filing_identity_hash', (p_filing ->> 'asset_id')::UUID,
        p_filing ->> 'ticker', p_filing ->> 'issuer_legal_name', p_filing ->> 'exchange',
        p_filing ->> 'company_type', p_filing ->> 'source_authority', p_filing ->> 'source_url',
        NULLIF(p_filing ->> 'source_disclosure_id', ''), p_filing ->> 'source_title',
        ...
        p_filing ->> 'statement_scope', p_filing ->> 'audit_status',
        NULLIF(p_filing ->> 'accounting_regime', ''),
        (p_filing ->> 'fiscal_year')::INTEGER,
        NULLIF(p_filing ->> 'fiscal_quarter', '')::SMALLINT,
        (p_filing ->> 'period_start')::DATE, (p_filing ->> 'period_end')::DATE,
        p_filing ->> 'period_kind', (p_filing ->> 'revision_number')::INTEGER,
        v_supersedes_id, NULLIF(p_filing ->> 'document_hash', ''),
        p_filing ->> 'verification_status', p_filing ->> 'methodology_version'
    );
    ...
    LOOP
        INSERT INTO public.vn_equity_fundamental_facts (
            ...
            (v_fact ->> 'id')::UUID, v_id, v_fact ->> 'metric_code',
            NULLIF(v_fact ->> 'source_line_code', ''), v_fact ->> 'source_label',
            NULLIF(v_fact ->> 'numeric_value', '')::NUMERIC, v_fact ->> 'currency_code',
            (v_fact ->> 'unit_scale')::BIGINT,
            NULLIF(v_fact ->> 'source_page', '')::INTEGER,
            NULLIF(v_fact ->> 'source_sheet', ''), NULLIF(v_fact ->> 'source_cell', ''),
            v_fact ->> 'value_kind', NULLIF(v_fact ->> 'derivation_formula', ''),
            (v_fact ->> 'confidence')::NUMERIC, v_fact ->> 'validation_status',
