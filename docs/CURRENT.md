...
The direct CoinGecko USDT/VND accounting provider remains fail-closed and disabled by default through `COINGECKO_ACCOUNTING_RATE_ENABLED=false`. Provider failure or disablement leaves optional VND enrichment unavailable and does not block an eligible native transaction.
...
- Native unrealized P/L is exposed only with a current price in exactly the native cost currency. Binance USDT is authoritative for a USDT-native current value; it is never relabelled USD. A USDT value reaches VND only through an actual direct USDT/VND quote.
