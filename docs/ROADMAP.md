...
- **COMPLETE (Native-First Accounting)**: External non-VND BUY/SELL no longer requires user-supplied VND accounting. Nullable VND price/P&L, native weighted holding cost, immutable reversal snapshots, direct-current-currency valuation, and missing-not-zero performance semantics are enforced across DB, API, and UI. VND/internal-cash requirements remain unchanged.
