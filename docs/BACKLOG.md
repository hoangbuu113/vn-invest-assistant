...
| P1 | Performance request efficiency | Ever-held asset identities and shared valuations | Fetch only required histories; reuse one performance result | Cash-only requests make zero asset-history calls; benchmark does not recompute portfolio performance independently | DEFECT |
