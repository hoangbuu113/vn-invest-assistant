...
function TransactionSettlementControls({
  ...
  const options = [
    {
      ...
      title: 'Giao dịch qua ví / sàn / tài khoản bên ngoài',
      description: 'Không thay đổi số dư tiền mặt VND đang theo dõi trong ứng dụng.'
    },
    {
      value: 'INTERNAL_VND_CASH',
      title: 'Thanh toán từ tiền mặt VND đang theo dõi',
      description: 'Mua sẽ trừ tiền mặt VND, Bán sẽ cộng thêm vào tiền mặt VND.'
    }
  ];
      ...
      {options.map((option) => (
        <label
          ...
          style={{
            ...
            border: `1px solid ${settlementMode === option.value ? 'var(--color-brand-600, #2563eb)' : 'var(--color-slate-200, #e2e8f0)'}`,
            borderRadius: '10px',
            backgroundColor: settlementMode === option.value ? 'rgba(37, 99, 235, 0.04)' : '#ffffff',
            cursor: 'pointer'
          }}
        >
          <input
            ...
            onChange={() => setSettlementMode(option.value)}
            disabled={loading}
            style={{ marginTop: '2px' }}
          />
          <div>
  ...
  if (compact) {
    return (
      ...
      >
        <summary style={{ padding: '0.7rem 0.85rem', cursor: 'pointer', fontSize: '0.8rem', color: 'var(--color-slate-600)' }}>
          Thanh toán: <strong style={{ color: 'var(--color-slate-800)' }}>Ví / sàn bên ngoài</strong>
          <span style={{ float: 'right', color: 'var(--color-brand-600, #2563eb)', fontWeight: 600 }}>Thay đổi</span>
        </summary>
...
export default function TransactionModal({
  ...
  const [accountingRateRefreshVersion, setAccountingRateRefreshVersion] = useState(0);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const initialAsset = (assets || []).find((asset) => (
        (asset.symbol || '').toUpperCase() === initialSymbol.toUpperCase()
      ));
      const initialDefaults = getTransactionEntryDefaults(initialAsset);
      ...
      setPriceCurrency(initialDefaults.priceCurrency);
      ...
      setAccountingRateRefreshVersion(0);
  ...
  const isAutomaticUsdtAccounting = shouldResolveUsdtVndAccountingRate({
    isCrypto,
    settlementMode,
    priceCurrency
  });
  const selectedAssetIdentity = selectedAssetObject?.id
    || selectedAssetObject?.asset_id
    || selectedSymbol.trim().toUpperCase();
  const selectedAssetContractKey = [
    selectedAssetIdentity,
    selectedAssetObject?.asset_type || selectedAssetObject?.assetType || '',
    selectedAssetObject?.quote_currency || selectedAssetObject?.quoteCurrency || ''
  ].join('|');
  const accountingRateIntentKey = useMemo(() => buildUsdtVndAccountingRateIntentKey({
    assetId: selectedAssetIdentity,
    assetSymbol: selectedSymbol,
    priceCurrency,
    settlementMode,
    executionUnitPrice,
    isCustomTime,
    executedAt: customDateTime
  }), [
    selectedAssetIdentity,
    selectedSymbol,
    priceCurrency,
  ...
  const automaticAccountingPrice = deriveUsdtVndAccountingPrice(
    executionUnitPrice,
    accountingRateState
  );
  const accountingRatePresentation = getUsdtVndAccountingPresentation({
    isAutomatic: isAutomaticUsdtAccounting,
    isSimplifiedCryptoExternal,
    status: accountingRateState.status
  });
  ...
  // Invalidate a dispatched attempt whenever the semantic rate intent changes.
  useEffect(() => {
    if (!isOpen) {
      previousAccountingRateIntentKeyRef.current = null;
      return;
  ...
  // Set currency and settlement defaults when the selected asset contract changes.
  // A refreshed assets array with identical semantics must not reset the form.
  useEffect(() => {
    if (!selectedAssetObject) return;
    const defaults = getTransactionEntryDefaults(selectedAssetObject);
    setPriceCurrency(defaults.priceCurrency);
    setSettlementMode(defaults.settlementMode);
    setPriceInput('');
    setExecutionUnitPrice('');
    setSettlementCurrency('');
    setIsFxPrefillConfirmed(false);
  ...
  useEffect(() => {
    if (!isOpen || !isAutomaticUsdtAccounting) {
      setAccountingRateState({
        status: ACCOUNTING_RATE_UI_STATUS.IDLE,
        quote: null,
        reason: null
    ...
    if (isCustomTime) {
      if (!customDateTime || !Number.isFinite(new Date(customDateTime).getTime())) {
        setAccountingRateState({
          status: ACCOUNTING_RATE_UI_STATUS.IDLE,
          quote: null,
    ...
    apiFetch(buildUsdtVndAccountingRatePath(requestedAt), { signal: controller.signal })
      .then(async (response) => {
        ...
        return normalizeUsdtVndAccountingRate(json.data);
      })
      .then((nextState) => {
        if (!active) return;
    ...
    accountingRateIntentKey,
    accountingRateRefreshVersion
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (normalizedExecutedAt) payload.executedAt = normalizedExecutedAt;
    ...
    pendingSubmissionIntentRef.current = intent;
    idempotencyKeyRef.current = intent.idempotencyKey;
    await dispatchFrozenSubmission(intent);
  };
      ...
      >
        ...
        <div style={{ overflowY: 'auto', padding: '1.25rem 1.5rem', flex: 1 }}>
          <form onSubmit={handleSubmit} id="transaction-entry-form">
            ...
            {isNonVnd && (
              ...
              >
                <div style={{ fontSize: '0.82rem', fontWeight: 800, color: 'var(--color-slate-800)', marginBottom: '0.75rem' }}>
                  {isSimplifiedCryptoExternal ? 'Giá giao dịch' : 'Thông tin giá thực hiện ngoại tệ'}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '10px', marginBottom: '0.75rem' }}>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 700, color: 'var(--color-slate-600)', marginBottom: '0.35rem' }}>
                      {isSimplifiedCryptoExternal ? 'Đồng giá' : 'Đồng tiền giá'}
                    </label>
                    <select
                      value={priceCurrency}
                      onChange={(e) => {
                ...
                {isSimplifiedCryptoExternal && (
                  <div
                    data-testid="crypto-native-total"
                    style={{
                  ...
                  >
                    ...
                    <strong style={{ fontSize: '0.92rem', color: 'var(--color-brand-700, #1d4ed8)' }}>
                      {nativeTransactionTotalLabel}
                    </strong>
                  </div>
                )}
            ...
            {isSimplifiedCryptoExternal && shouldShowAutomaticAccounting && (
              <div
                data-testid="automatic-usdt-vnd-accounting"
                style={{
                  marginBottom: '1.25rem',
              ...
              >
                {accountingRateState.status === ACCOUNTING_RATE_UI_STATUS.AVAILABLE ? (
                  <>
                    <div style={{ fontWeight: 700 }}>
                      Giá hạch toán VND được tự động tính theo dữ liệu CoinGecko (tỷ giá USDT/VND trực tiếp).
                    </div>
                    <details style={{ marginTop: '0.35rem' }}>
                      <summary style={{ cursor: 'pointer', color: 'var(--color-brand-700, #1d4ed8)', fontWeight: 600 }}>
                        Chi tiết quy đổi
                      </summary>
                      <div style={{ marginTop: '0.35rem', color: 'var(--color-slate-600)' }}>
                        <div>
                          1 USDT = {accountingRateState.quote.rate.toLocaleString('vi-VN', { maximumFractionDigits: 8 })} VND
                        </div>
                        <div>Nguồn: {accountingRateState.quote.provider}</div>
                        {Number.isFinite(automaticAccountingPrice) && (
                          <div>
                            Giá hạch toán: {automaticAccountingPrice.toLocaleString('vi-VN', { maximumFractionDigits: 8 })} ₫/đơn vị
                          </div>
                        )}
                        <div>
                          Quan sát lúc {new Date(accountingRateState.quote.observedAt).toLocaleString('vi-VN')}
                        </div>
                      </div>
                    </details>
                  </>
            ...
            {!isSimplifiedCryptoExternal && Number(quantityInput) > 0 && Number(priceInput) > 0 && (
              <div
                style={{
                  padding: '0.65rem 0.85rem',
                  backgroundColor: 'rgba(37, 99, 235, 0.05)',
